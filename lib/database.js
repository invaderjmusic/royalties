...
module.exports = {
    ...
    getDetailedUsersInfo: async () => {
        ...
        using name := .user.username 
        by name),
        payouts := (group Payout {
            amount,
        } 
        using name := .user.username 
        by name)
        ...
        select earnings {
            name := .key.name,
            earnings := sum(
                (for earning in .elements
                union earning.amount)
            ),
            payouts := sum(
                (with userpayouts := (
                select payouts
                filter payouts.key.name = earnings.key.name
                )
            for payout in userpayouts.elements
            union payout.amount)
            ),
            details := (
                select User {
            ...
            filter User.username = earnings.key.name
            )
        }
        `)
        return result;
