...
async function getData() {
    let error = false;
    try {
        let response = await fetch(`/api/listUserSalePayouts?page=1`);
...
function parseResdata() {
    if (resdata.length == 0) {
        if (page == 1) {
            document.getElementById("nopayouts").style.display = "block";
...
async function loadNewData() {
    ...
    try {
        let response = await fetch(`/admin/listUserSalePayouts?page=${page.toString()}`);
        resdata = await response.json();
        parseResdata();
    }
    catch (err) {
